...
def test_mountinfo_separator_and_escaped_path(tmp_path):
    ...
    p.write_text(
        "31 1 259:2 / / rw - ext4 /dev/nvme0n1p2 rw\n"
...
def test_smi_uma_na_is_explicit_not_zero(monkeypatch):
    ...
    def run(argv, **kwargs):
        ...
        return SimpleNamespace(
            returncode=0,
            stdout="0, NVIDIA GB10, GPU-fixture, 0, [N/A]\n" if len(seen) == 1 else "",
        )
...
def test_wrong_gpu_model_rejected():
    gpu = {
        "gpus": [{**GPU_ONE_IDLE["gpus"][0], "name": "NVIDIA RTX 5090"}],
        "compute_apps": [],
    }
...
def test_uncreated_nested_cache_uses_existing_filesystem(tmp_path):
    ...
    got = pf._fs_identity(
        str(tmp_path / "not-yet" / "ple" / "revision"), [entry], pf.statvfs
    )
...
def test_named_fuse_subtype_rejected():
    assert (
        _runner(
            ple_dir="/mnt/ple",
            mounts=[
                {"mount_point": "/", "fstype": "fuse.sshfs", "source": "test-only"}
            ],
            free={"test-only": 1 << 40},
        )()["status"]
        == "FAIL"
