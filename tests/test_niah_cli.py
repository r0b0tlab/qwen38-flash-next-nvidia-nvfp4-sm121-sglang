...
def small_case():
    ...
    case = nz.build_case(tokenizer, case_id='test_cli', n_prompt_tokens=512,
                         depths=(0.5,), codes=('ZEPHYR-4821',), query='Return the code.')
...
def test_live_cli_constructs_and_verifies_client_before_one_run(tmp_path, monkeypatch):
    ...
    calls = []
    class Client:
        def __init__(self, base): calls.append('construct')
        def verify_model(self): calls.append('verify')
    def run(client, cases, out, *, dry_run=False, on_row=None):
    ...
    rc = nz.main(['--base','http://unit.test','--tokenizer-dir',str(tmp_path),
                  '--manifest',str(tmp_path/'manifest.json'),'--output',str(tmp_path/'rows.jsonl')])
    assert rc == 0
...
def test_dry_run_success_is_not_retrieval_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(nz, 'load_real_tokenizer', lambda path: tokenizer)
    monkeypatch.setattr(nz, 'build_default_cases', lambda tok: [case])
    def forbid(*args, **kwargs): raise AssertionError('dry run constructed HTTP client')
    monkeypatch.setattr(nz, 'OpenAICompatClient', forbid)
    rc = nz.main(['--dry-run','--tokenizer-dir',str(tmp_path),
                  '--manifest',str(tmp_path/'manifest.json'),'--output',str(tmp_path/'rows.jsonl')])
